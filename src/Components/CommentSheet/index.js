...
class CommentSheet extends React.Component {
  ...
  getInputHeight = () => {
    ...
    if (
      this.state.currentMessage?.length > 40 ||
  ...
  addComment = async () => {
    ...
    try {
      this.setState({
        comments: [...this.state.comments, {...dataToStoreLocally}],
        currentMessage: '',
      });
  ...
  deleteComment = async comment_id => {
    ...
    try {
      let commentIndex = this.state.comments?.findIndex(
        _comment => _comment.id == comment_id,
      );
  ...
  onUpdate = async title => {
    ...
    try {
      let index = this.state.comments.findIndex(
        comment => comment?.id == this.state.activeComment?.id,
      );
        ...
        <View style={styles.footer}>
          ...
          <View style={styles.iconView}>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Image source={allImages.generalIcons.send} style={styles.icon} />
            </TouchableOpacity>
          </View>
        </View>
      </View>
  ...
  render() {
    return this._renderBottomSheet();
  }
}
...
const mapDispatchToProps = dispatch => {
  return {
    createComment: (data, dataToStoreLocally) =>
      dispatch(actions.createComment(data, dataToStoreLocally)),
