...
import {showToast} from '../../Api/HelperFunctions.js';
...
class PoemFeedCard extends Component {
  ...
  renderLikers = () => {
    if (this.props.likers?.length > 0) {
      let likers = [...this.props.likers].reverse();

      if (likers.length == 1) {
        return (
          <TouchableOpacity onPress={this.showLikers} activeOpacity={0.7}>
  ...
  renderCommentBubble = () => {
    ...
    return (
      <View style={styles.bubble}>
        <Text style={styles.bubbleText}>{this.props?.comments?.length}</Text>
      </View>
    );
  };
      ...
      <View style={styles.ripple}>
        <View style={{marginVertical: 0.3 * vh}}>
          <View style={styles.topRow}>
            ...
            >
              <Image source={this.props.source} style={styles.profileImage} />

              <View>
                <Text style={styles.name}>{this.props.name}</Text>
                <Text style={styles.date}>{this.props.created_at}</Text>
              </View>
            </TouchableOpacity>

            {this.props.showOptions && (
              <TouchableOpacity
                onPress={this.showOptionSheet}
                activeOpacity={0.7}
              >
                <Image
        ...
        <View style={{marginTop: 1 * vh}}>
          <Text style={styles.heading}>Verses:</Text>
          {this.renderVerses()}
        </View>
        ...
        <View style={{marginVertical: 0 * vh}}>
          <View style={styles.bottomRow}>
            <TouchableOpacity onPress={this.toggleLike} activeOpacity={0.7}>
              <Image source={this.getActiveIcon()} style={styles.image} />
            </TouchableOpacity>
