...
class FeedScreen extends React.Component {
  ...
  FriendComponent = _friend => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => this.onFriendPress(_friend?._id)}
        style={styles.friendImageContainer}
      >
        <View style={styles.friendImageView}>
          <Image source={getProfileImage(_friend)} style={styles.friendImage} />
        </View>
  ...
  getFriends = () => {
    ...
    const index = friends?.findIndex(
      friend => friend?._id == this.props.profile?._id,
    );
    if (index != -1) {
      ...
      <View style={styles.friendsContainer}>
        <View style={styles.friendsTopRow}>
...

          {
            <TouchableOpacity
              accessibilityRole="button"
              onPress={this.navigateToAllUsers}
            >
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        />
        <LoginPopUp
          ref={_ref => (this.loginPopupRef = _ref)}
...
const keyExtractor = (item, _) => `feedCard-${String(item._id)}`;
